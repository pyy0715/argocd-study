...
소스 수정 → push
  └─ CI: build → registry push
      └─ CI: manifest repo 의 image 필드를 새 tag 로 수정 → PR (or 직접 commit)
          └─ (PR 머지)
              └─ ArgoCD refresh → 새 이미지로 sync
```
...
```
이미지가 registry 에 푸시 (CI든 수동이든 외부팀이든)
  └─ Image Updater Pod 가 polling
      └─ 새 tag 발견 → writeBackTarget 대상 파일 commit
          └─ ArgoCD refresh → 새 이미지로 sync
```
...
```
이미지가 registry 에 푸시
  └─ Deployment 가 image: ...:latest 처럼 mutable tag 를 참조
      └─ imagePullPolicy: Always + rollout 재실행 시 자동으로 새 이미지 pull
```
